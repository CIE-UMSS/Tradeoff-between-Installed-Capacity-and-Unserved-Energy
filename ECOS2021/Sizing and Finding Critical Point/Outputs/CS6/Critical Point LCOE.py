...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 08:08:52 2021
...
import pandas as pd
BSA=pd.read_csv('CS6.csv', sep=',', decimal='.', encoding='latin1')
...
r=list()
for a in np.arange(-0.8,1.1,0.01):
    l1=m1*x+a
    z2=np.array([m1, a])
    p2=np.poly1d(z2)
    eq1=Eq(l1-j)
...
rt['step']=np.arange(-0.8,1.1,0.01)
...
CS6f=popt
CS6r2=r_squared
CS6x=rt['x1'].iloc[rt['d'].idxmax()]
CS6y=rt['y1'].iloc[rt['d'].idxmax()]
...
#plt.plot(x_1, y_1, '-')
plt.plot(CS6x,CS6y, marker='o', markersize=5, color="red", label='critical point')
#escala real
...
print('R2=',CS6r2)
print('parameters=',CS6f)
print('critical point=',CS6x)
