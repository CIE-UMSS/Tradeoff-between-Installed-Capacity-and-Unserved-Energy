...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 23:21:41 2021
...
import pandas as pd
BSA=pd.read_csv('S12.csv', sep=',', decimal='.', encoding='latin1')
df=pd.concat([BSA['LLC'], BSA['NPC (USD)']], axis=1, keys=['LLP', 'NPC'])
df['NPC']=df['NPC']/1000
from sklearn.linear_model import LinearRegression
import matplotlib.pyplot as plt
from scipy.optimize import curve_fit
def func12(x, a, b, c,d,e):
    return (a*x-b)/(c*x**e+d)
xdata = df['LLP']
ydata=df['NPC']
plt.figure(figsize=(10,10))
plt.plot(xdata, ydata, 'b-', label='data')
popt, pcov = curve_fit(func12, xdata, ydata)
popt

residuals = ydata- func12(xdata, *popt)
...
S12f=popt
S12r2=r_squared
S12x=rt['x1'].iloc[rt['d'].idxmax()]
S12y=rt['y1'].iloc[rt['d'].idxmax()]
...
o= plt.plot(xp, func12(xp,*popt), '--', label='fit', color='green')
...
#plt.plot(x_1, y_1, '-')
plt.plot(S12x,S12y, marker='o', markersize=5, color="red", label='critical point')
#escala real
