...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 22:59:40 2021
...
import pandas as pd
BSA=pd.read_csv('S9.csv', sep=',', decimal='.', encoding='latin1')
df=pd.concat([BSA['LLC'], BSA['NPC (USD)']], axis=1, keys=['LLP', 'NPC'])
df['NPC']=df['NPC']/1000
from sklearn.linear_model import LinearRegression
import matplotlib.pyplot as plt
from scipy.optimize import curve_fit
def func9(x, a, b, c,d,e,f):
    return (a*x-b)/(c*x+d+e*x**f)
xdata = df['LLP']
ydata=df['NPC']
popt, pcov = curve_fit(func9, xdata, ydata)

residuals = ydata- func9(xdata, *popt)
...
f=(popt[0]*x-popt[1])/(popt[2]*x+popt[3]+popt[4]*x**popt[5])
#eq1=Eq(l1-j)
eq3=Eq(f-j)
...
S9f=popt
S9r2=r_squared
S9x=rt['x1'].iloc[rt['d'].idxmax()]
S9y=rt['y1'].iloc[rt['d'].idxmax()]


plt.figure(figsize=(10,6.7))
xp = np.linspace(0,1, 100)
_ = plt.plot(x_1, y_1, '.',label='data', color='blue')
o= plt.plot(xp, func9(xp,*popt), '--', label='fit', color='green')
...
#plt.plot(x_1, y_1, '-')
plt.plot(S9x,S9y, marker='o', markersize=5, color="red", label='critical point')
#escala real
