...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 00:19:03 2021
...
BSA=pd.read_csv('S16.csv', sep=',', decimal='.', encoding='latin1')
...
popt, pcov = curve_fit(func3, xdata, ydata, bounds=(-0.2, [20, 0, 20]))
popt
residuals = ydata- func3(xdata, *popt)
ss_res = np.sum(residuals**2)
ss_tot = np.sum((ydata-np.mean(ydata))**2)
...
r=list()
for a in np.arange(-0.16,5.5,0.01):
    l1=m1*x+a
    z2=np.array([m1, a])
    p2=np.poly1d(z2)
    eq1=Eq(l1-j)
...
rt['step']=np.arange(-0.16,5.5,0.01)
...
S16f=popt
S16r2=r_squared
S16x=rt['x1'].iloc[rt['d'].idxmax()]
S16y=rt['y1'].iloc[rt['d'].idxmax()]


plt.figure(figsize=(10,6.7))
xp = np.linspace(0,1, 100)
_ = plt.plot(x_1, y_1, '.',label='data', color='blue')
o= plt.plot(xp, func3(xp,*popt), '--', label='fit', color='green')
...
#plt.plot(x_1, y_1, '-')
plt.plot(S16x,S16y, marker='o', markersize=5, color="red", label='critical point')
#escala real
