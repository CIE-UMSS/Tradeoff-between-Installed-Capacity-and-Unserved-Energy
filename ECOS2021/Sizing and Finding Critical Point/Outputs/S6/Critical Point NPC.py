...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 21:41:36 2021
...
import pandas as pd
BSA=pd.read_csv('S6.csv', sep=',', decimal='.', encoding='latin1')
df=pd.concat([BSA['LLC'], BSA['NPC (USD)']], axis=1, keys=['LLP', 'NPC'])
df['NPC']=df['NPC']/1000
from sklearn.linear_model import LinearRegression
import matplotlib.pyplot as plt
from scipy.optimize import curve_fit
def func6(x, a, b, c,d,e,f,g,h):
    return (a*x-b)/(c*x**e+d+x**f+x**g+x**h)
xdata = df['LLP']
ydata=df['NPC']
popt, pcov = curve_fit(func6, xdata, ydata)
popt

residuals = ydata- func6(xdata, *popt)
...
for a in np.arange(liminf,limsup,0.01):
    ...
    eq1=Eq(l1-j)
    eq3=Eq(f-j)
    sol1 = nsolve((eq1, eq3), (x,j), (0.005, 1.5))  
    r.append([sol1])
r=pd.DataFrame(r)
...
S6f=popt
S6r2=r_squared
S6x=rt['x1'].iloc[rt['d'].idxmax()]
S6y=rt['y1'].iloc[rt['d'].idxmax()]


plt.figure(figsize=(10,6.7))
xp = np.linspace(0,1, 100)
_ = plt.plot(x_1, y_1, '.',label='data', color='blue')
o= plt.plot(xp, func6(xp,*popt), '--', label='fit', color='green')
...
#plt.plot(x_1, y_1, '-')
plt.plot(S6x,S6y, marker='o', markersize=5, color="red", label='critical point')
#escala real
