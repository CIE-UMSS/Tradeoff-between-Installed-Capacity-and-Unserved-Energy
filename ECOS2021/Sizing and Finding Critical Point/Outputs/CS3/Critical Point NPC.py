...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 07:52:39 2021
...
import pandas as pd
BSA=pd.read_csv('CS3.csv', sep=',', decimal='.', encoding='latin1')
...
r=list()
for a in np.arange(-0.48,1.7,0.01):
    l1=m1*x+a
    z2=np.array([m1, a])
    p2=np.poly1d(z2)
    eq1=Eq(l1-j)
...
rt['step']=np.arange(-0.48,1.7,0.01)
...
CS3f=popt
CS3r2=r_squared
CS3x=rt['x1'].iloc[rt['d'].idxmax()]
CS3y=rt['y1'].iloc[rt['d'].idxmax()]
...
#plt.plot(x_1, y_1, '-')
plt.plot(CS3x,CS3y, marker='o', markersize=5, color="red", label='critical point')
#escala real
...
print('R2=',CS3r2)
print('parameters=',CS3f)
print('critical point=',CS3x)
