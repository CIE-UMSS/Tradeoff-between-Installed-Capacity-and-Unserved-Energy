...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 23:03:10 2021
...
import pandas as pd
BSA=pd.read_csv('S10.csv', sep=',', decimal='.', encoding='latin1')
df=pd.concat([BSA['LLC'], BSA['NPC (USD)']], axis=1, keys=['LLP', 'NPC'])
df['NPC']=df['NPC']/1000
from sklearn.linear_model import LinearRegression
import matplotlib.pyplot as plt
from scipy.optimize import curve_fit
def func10(x, a, b, c,d,e):
    return (a*x-b)/(c*x**e+d)
xdata = df['LLP']
ydata=df['NPC']
plt.figure(figsize=(10,10))
plt.plot(xdata, ydata, 'b-', label='data')
popt, pcov = curve_fit(func10, xdata, ydata)
popt

residuals = ydata- func10(xdata, *popt)
ss_res = np.sum(residuals**2)
ss_tot = np.sum((ydata-np.mean(ydata))**2)
r_squared = 1 - (ss_res / ss_tot)

popt1, pcov = curve_fit(func10, xdata, ydata,bounds=(-0.2, [20, 0, 300, 2.1, 0.4]))

residuals = ydata- func10(xdata, *popt1)
ss_res = np.sum(residuals**2)
ss_tot = np.sum((ydata-np.mean(ydata))**2)
r_squared = 1 - (ss_res / ss_tot)
...
S10f=popt
S10r2=r_squared
S10x=rt['x1'].iloc[rt['d'].idxmax()]
S10y=rt['y1'].iloc[rt['d'].idxmax()]
...
o= plt.plot(xp, func10(xp,*popt), '--', label='fit', color='green')
...
#plt.plot(x_1, y_1, '-')
plt.plot(S10x,S10y, marker='o', markersize=5, color="red", label='critical point')
#escala real
