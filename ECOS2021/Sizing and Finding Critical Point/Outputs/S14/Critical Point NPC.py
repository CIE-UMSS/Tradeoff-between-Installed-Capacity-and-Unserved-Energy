...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 00:07:45 2021
...
import pandas as pd
BSA=pd.read_csv('S14.csv', sep=',', decimal='.', encoding='latin1')
...
def func14(x, a, b, c,d,e):
    return (a*x-b)/(c*x**e+d)
xdata = df['LLP']
ydata=df['NPC']
popt, pcov = curve_fit(func14, xdata, ydata)
popt

residuals = ydata- func14(xdata, *popt)
...
S14f=popt
S14r2=r_squared
S14x=rt['x1'].iloc[rt['d'].idxmax()]
S14y=rt['y1'].iloc[rt['d'].idxmax()]
...
o= plt.plot(xp, func14(xp,*popt), '--', label='fit', color='green')
...
#plt.plot(x_1, y_1, '-')
plt.plot(S14x,S14y, marker='o', markersize=5, color="red", label='critical point')
#escala real
