...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 23:17:30 2021
...
import pandas as pd
BSA=pd.read_csv('S11.csv', sep=',', decimal='.', encoding='latin1')
df=pd.concat([BSA['LLC'], BSA['NPC (USD)']], axis=1, keys=['LLP', 'NPC'])
df['NPC']=df['NPC']/1000
from sklearn.linear_model import LinearRegression
import matplotlib.pyplot as plt
from scipy.optimize import curve_fit
def func11(x, a, b, c,d,e,f,g, h, i):
    return (a*x-b)/(c*x**e+d+g*x**f+h*x**i)
xdata = df['LLP']
ydata=df['NPC']

popt, pcov = curve_fit(func11, xdata, ydata)


residuals = ydata- func11(xdata, *popt)
...
S11f=popt
S11r2=r_squared
S11x=rt['x1'].iloc[rt['d'].idxmax()]
S11y=rt['y1'].iloc[rt['d'].idxmax()]
...
o= plt.plot(xp, func11(xp,*popt), '--', label='fit', color='green')
...
#plt.plot(x_1, y_1, '-')
plt.plot(S11x,S11y, marker='o', markersize=5, color="red", label='critical point')
#escala real
