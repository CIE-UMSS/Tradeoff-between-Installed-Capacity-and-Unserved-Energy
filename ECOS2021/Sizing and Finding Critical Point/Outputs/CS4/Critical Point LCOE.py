...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 07:54:59 2021
...
import pandas as pd
BSA=pd.read_csv('CS4.csv', sep=',', decimal='.', encoding='latin1')
...
f=(popt[0]*x+popt[1])/(popt[2]+popt[4]*x**popt[3])
#(a*x+b)/(c+e*x**d)
#eq1=Eq(l1-j)
eq3=Eq(f-j)
...
r=list()
for a in np.arange(-0.85,1.1,0.01):
    l1=m1*x+a
    z2=np.array([m1, a])
    p2=np.poly1d(z2)
    eq1=Eq(l1-j)
    eq3=Eq(f-j)
    sol1 = nsolve((eq1, eq3), (x,j), (0.005, 1))  
    r.append([sol1])
r=pd.DataFrame(r)
...
rt['step']=np.arange(-0.85,1.1,0.01)
...
CS4f=popt
CS4r2=r_squared
CS4x=rt['x1'].iloc[rt['d'].idxmax()]
CS4y=rt['y1'].iloc[rt['d'].idxmax()]
...
#plt.plot(x_1, y_1, '-')
plt.plot(CS4x,CS4y, marker='o', markersize=5, color="red", label='critical point')
#escala real
...
print('R2=',CS4r2)
print('parameters=',CS4f)
print('critical point=',CS4x)
