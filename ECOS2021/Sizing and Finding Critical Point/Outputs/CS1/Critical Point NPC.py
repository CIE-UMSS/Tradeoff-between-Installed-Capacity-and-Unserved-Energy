...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 07:33:04 2021
...
import pandas as pd
BSA=pd.read_csv('CS1.csv', sep=',', decimal='.', encoding='latin1')
...
r=list()
for a in np.arange(-0.46,1.7,0.01):
    l1=m1*x+a
    z2=np.array([m1, a])
    p2=np.poly1d(z2)
    eq1=Eq(l1-j)
...
rt['step']=np.arange(-0.46,1.7,0.01)
...
CS1x=rt['x1'].iloc[rt['d'].idxmax()]
CS1y=rt['y1'].iloc[rt['d'].idxmax()]
...
#plt.plot(x_1, y_1, '-')
plt.plot(CS1x,CS1y, marker='o', markersize=5, color="red", label='critical point')
#escala real
