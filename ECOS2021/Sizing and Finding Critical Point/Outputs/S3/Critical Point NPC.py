...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 21:25:06 2021
...
import pandas as pd
BSA=pd.read_csv('S3.csv', sep=',', decimal='.', encoding='latin1')
...
#(a*x-b)/(c*x**e+d+g*x**f)
f=(popt[0]*x-popt[1])/(popt[2]*x**popt[4]+popt[3]+popt[6]*x**popt[5])
#eq1=Eq(l1-j)
eq3=Eq(f-j)
...
S3f=popt
S3r2=r_squared
S3x=rt['x1'].iloc[rt['d'].idxmax()]
S3y=rt['y1'].iloc[rt['d'].idxmax()]
...
#plt.plot(x_1, y_1, '-')
plt.plot(S3x,S3y, marker='o', markersize=5, color="red", label='critical point')
#escala real
