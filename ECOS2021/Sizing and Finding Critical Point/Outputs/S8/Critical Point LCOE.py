...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 22:50:52 2021
...
BSA=pd.read_csv('S8.csv', sep=',', decimal='.', encoding='latin1')
...
r=list()
for a in np.arange(-0.2,1.5,0.01):
    l1=m1*x+a
    z2=np.array([m1, a])
    p2=np.poly1d(z2)
    eq1=Eq(l1-j)
...
rt['step']=np.arange(-0.2,1.5,0.01)
...
S8f=popt
S8r2=r_squared
S8x=rt['x1'].iloc[rt['d'].idxmax()]
S8y=rt['y1'].iloc[rt['d'].idxmax()]

plt.figure(figsize=(10,6.7))
xp = np.linspace(0,1, 100)
_ = plt.plot(x_1, y_1, '.',label='data', color='blue')
o= plt.plot(xp, func1(xp,*popt), '--', label='fit', color='green')
...
#plt.plot(x_1, y_1, '-')
plt.plot(S8x,S8y, marker='o', markersize=5, color="red", label='critical point')
#escala real
