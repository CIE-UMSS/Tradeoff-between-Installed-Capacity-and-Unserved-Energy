...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 07:44:48 2021
...
import pandas as pd
BSA=pd.read_csv('CS2.csv', sep=',', decimal='.', encoding='latin1')
...
r=list()
for a in np.arange(-0.48,1.8,0.01):
    l1=m1*x+a
    z2=np.array([m1, a])
    p2=np.poly1d(z2)
    eq1=Eq(l1-j)
...
rt['step']=np.arange(-0.48,1.8,0.01)
...
CS2f=popt
CS2r2=r_squared
CS2x=rt['x1'].iloc[rt['d'].idxmax()]
CS2y=rt['y1'].iloc[rt['d'].idxmax()]
...
#plt.plot(x_1, y_1, '-')
plt.plot(CS2x,CS2y, marker='o', markersize=5, color="red", label='critical point')
#escala real
...
print('R2=',CS2r2)
print('parameters=',CS2f)
print('critical point=',CS2x)
