...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 08:26:25 2021
...
import pandas as pd
BSA=pd.read_csv('CS8.csv', sep=',', decimal='.', encoding='latin1')
...
r=list()
for a in np.arange(-0.75,1.1,0.01):
    l1=m1*x+a
    z2=np.array([m1, a])
    p2=np.poly1d(z2)
    eq1=Eq(l1-j)
...
rt['step']=np.arange(-0.75,1.1,0.01)
...
CS8f=popt
CS8r2=r_squared
CS8x=rt['x1'].iloc[rt['d'].idxmax()]
CS8y=rt['y1'].iloc[rt['d'].idxmax()]
...
#plt.plot(x_1, y_1, '-')
plt.plot(CS8x,CS8y, marker='o', markersize=5, color="red", label='critical point')
#escala real
...
print('R2=',CS8r2)
print('parameters=',CS8f)
print('critical point=',CS8x)
