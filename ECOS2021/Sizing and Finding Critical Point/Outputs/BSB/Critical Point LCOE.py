...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 20:41:26 2021
...
BSA=pd.read_csv('BSB.csv', sep=',', decimal='.', encoding='latin1')
...
r=list()
for a in np.arange(df.LCOE.iloc[20]-m1*df.LLP.iloc[20],df.LCOE.iloc[0]-m1*df.LLP.iloc[0],0.01):
    l1=m1*x+a
    z2=np.array([m1, a])
    p2=np.poly1d(z2)
    eq1=Eq(l1-j)
...
rt['step']=np.arange(df.LCOE.iloc[20]-m1*df.LLP.iloc[20],df.LCOE.iloc[0]-m1*df.LLP.iloc[0],0.01)
...
BSBf=popt
BSBr2=r_squared
BSBx=rt['x1'].iloc[rt['d'].idxmax()]
BSBy=rt['y1'].iloc[rt['d'].idxmax()]
...
#plt.plot(x_1, y_1, '-')
plt.plot(BSBx,BSBy, marker='o', markersize=5, color="red", label='critical point')
#escala real
