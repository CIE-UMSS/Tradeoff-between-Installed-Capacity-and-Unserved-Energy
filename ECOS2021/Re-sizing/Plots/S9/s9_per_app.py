# -*- coding: utf-8 -*-
"""
Created on Tue Jan 19 22:18:32 2021
...
freezer=pd.read_csv('freezer_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
fridge=pd.read_csv('fridge_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
whelder=pd.read_csv('whelder_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
grinder=pd.read_csv('grinder_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
dryer=pd.read_csv('dryer_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
...
freezer=freezer.rename({'1': 'freezer'}, axis='columns')
fridge=fridge.rename({'1': 'fridge'}, axis='columns')
whelder=whelder.rename({'1': 'whelder'}, axis='columns')
grinder=grinder.rename({'1': 'grinder'}, axis='columns')
dryer=dryer.rename({'1': 'dryer'}, axis='columns')
...
del freezer['Unnamed: 0']
del fridge['Unnamed: 0']
del whelder['Unnamed: 0']
del grinder['Unnamed: 0']
del dryer['Unnamed: 0']

#demanda promedio (media horaria)
h_BSA=pd.read_csv('hourly_S1_A.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
del h_BSA['Unnamed: 0']

#llamamos a la perdida de carga
xls = pd.ExcelFile('S1_LCOE.xls')
sheetX=xls.parse(1)
...
#unimos todo el data
BSAhourly=pd.concat([LCOE_BSA, h_BSA,indoor_bulb,outdoor_bulb,radio,tv,phone_charger,water_heater,mixer,freezer,fridge,whelder,grinder,dryer], axis=1, sort=False)
...
BSAhourly['freezer'] = BSAhourly['freezer'].astype(float)
BSAhourly['fridge'] = BSAhourly['fridge'].astype(float)
BSAhourly['whelder'] = BSAhourly['whelder'].astype(float)
BSAhourly['grinder'] = BSAhourly['grinder'].astype(float)
BSAhourly['dryer'] = BSAhourly['dryer'].astype(float)

#NPC

#unimos todo el data
BSAhourly_1=pd.concat([NPC_BSA, h_BSA,indoor_bulb,outdoor_bulb,radio,tv,phone_charger,water_heater,mixer,freezer,fridge,whelder,grinder,dryer], axis=1, sort=False)
...
BSAhourly_1['freezer'] = BSAhourly_1['freezer'].astype(float)
BSAhourly_1['fridge'] = BSAhourly_1['fridge'].astype(float)
BSAhourly_1['whelder'] = BSAhourly_1['whelder'].astype(float)
BSAhourly_1['grinder'] = BSAhourly_1['grinder'].astype(float)
BSAhourly_1['dryer'] = BSAhourly_1['dryer'].astype(float)
...
l6,=axs[0].plot(mean['local_hour'], mean['water_heater'],linewidth=1)
...
#plt.fill_between(mean['local_hour'], mean['mixer'], color='rosybrown',alpha=.3)
#l9,=axs[0].plot(Oct['time_s'], Oct['0'],linewidth=1)
l9,=axs[0].plot(mean['local_hour'], mean['freezer'],linewidth=1)

l10,=axs[0].plot(mean['local_hour'], mean['fridge'],linewidth=1)


l11,=axs[0].plot(mean['local_hour'], mean['whelder'],linewidth=1)

l12,=axs[0].plot(mean['local_hour'], mean['grinder'],linewidth=1)
l13,=axs[0].plot(mean['local_hour'], mean['dryer'],linewidth=1)
...
axs[1].plot(mean_1['local_hour'], mean_1['water_heater'],linewidth=1)
...
axs[1].plot(mean_1['local_hour'], mean_1['freezer'],linewidth=1)
axs[1].plot(mean_1['local_hour'], mean_1['fridge'],linewidth=1)
axs[1].plot(mean_1['local_hour'], mean_1['whelder'],linewidth=1)
axs[1].plot(mean_1['local_hour'], mean_1['grinder'],linewidth=1)
axs[1].plot(mean_1['local_hour'], mean_1['dryer'],linewidth=1)
...
ax.legend()
ax.legend(handles = [l0,l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l13] , labels=['Demand', 'LL', 'indoor bulb', 'outdoor bulb', 'radio', 'tv', 'water heater', 'mixer','phone charger','freezer','fridge','whelder','grinder','dryer'],loc='upper right',bbox_to_anchor=(1.55, 0.98),fancybox=False, shadow=False, ncol=1)
plt.savefig('per_app_comp_s9.png',dpi=600,bbox_inches="tight")
plt.show()
