...
s1=pd.read_csv('hourly_freezer.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
s2=pd.read_csv('hourly_fridge.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
...
s1=s1.rename({'1': 's1'}, axis='columns')
s2=s2.rename({'1': 's2'}, axis='columns')
...
#unimos todo el data
BSAhourly=pd.concat([LCOE_BSA, h_BSA,indoor_bulb,outdoor_bulb,radio,tv,phone_charger,water_heater,mixer,s1,s2], axis=1, sort=False)
...
BSAhourly['s1'] = BSAhourly['s1'].astype(float)
BSAhourly['s2'] = BSAhourly['s2'].astype(float)

#NPC

#unimos todo el data
BSAhourly_1=pd.concat([NPC_BSA, h_BSA,indoor_bulb,outdoor_bulb,radio,tv,phone_charger,water_heater,mixer,s1,s2], axis=1, sort=False)
...
BSAhourly_1['s1'] = BSAhourly_1['s1'].astype(float)
BSAhourly_1['s2'] = BSAhourly_1['s2'].astype(float)
...
l9,=axs[0].plot(mean['local_hour'], mean['s1'],linewidth=1)

l10,=axs[0].plot(mean['local_hour'], mean['s2'],linewidth=1)
...
axs[1].plot(mean_1['local_hour'], mean_1['s1'],linewidth=1)

axs[1].plot(mean_1['local_hour'], mean_1['s2'],linewidth=1)
...
ax.legend()
ax.legend(handles = [l0,l1,l2,l3,l4,l5,l6,l7,l8,l9,l10] , labels=['Demand', 'LL', 'indoor bulb', 'outdoor bulb', 'radio', 'tv', 'water heater', 'mixer','phone charger','Freezer*', 'Fridge*'],loc='upper right',bbox_to_anchor=(1.55, 0.98),fancybox=False, shadow=False, ncol=1)
plt.savefig('per_app_comp_s5.png',dpi=600,bbox_inches="tight")
plt.show()
