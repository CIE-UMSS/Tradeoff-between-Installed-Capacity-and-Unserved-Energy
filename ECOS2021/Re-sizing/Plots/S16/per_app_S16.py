...
washing=pd.read_csv('washing_machine_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
water=pd.read_csv('water_pump_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
dvd=pd.read_csv('DVD_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
blender=pd.read_csv('blender_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
iron=pd.read_csv('iron_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
mill=pd.read_csv('mill_hourly.csv', sep=',', decimal=',', encoding='latin1', error_bad_lines=False)
...
washing=washing.rename({'1': 'washing'}, axis='columns')
water=water.rename({'1': 'water'}, axis='columns')
dvd=dvd.rename({'1': 'dvd'}, axis='columns')
blender=blender.rename({'1': 'blender'}, axis='columns')
iron=iron.rename({'1': 'iron'}, axis='columns')
mill=mill.rename({'1': 'mill'}, axis='columns')
...
del washing['Unnamed: 0']
del water['Unnamed: 0']
del dvd['Unnamed: 0']
del blender['Unnamed: 0']
del iron['Unnamed: 0']
del mill['Unnamed: 0']
...
#unimos todo el data
BSAhourly=pd.concat([LCOE_BSA, h_BSA,indoor_bulb,outdoor_bulb,radio,tv,phone_charger,washing,water,dvd,iron,blender,mill], axis=1, sort=False)
...
BSAhourly['washing'] = BSAhourly['washing'].astype(float)
BSAhourly['water'] = BSAhourly['water'].astype(float)
BSAhourly['dvd'] = BSAhourly['dvd'].astype(float)
BSAhourly['blender'] = BSAhourly['blender'].astype(float)
BSAhourly['iron'] = BSAhourly['iron'].astype(float)
BSAhourly['mill'] = BSAhourly['mill'].astype(float)
...
#unimos todo el data
BSAhourly_1=pd.concat([NPC_BSA, h_BSA,indoor_bulb,outdoor_bulb,radio,tv,phone_charger,washing,water,dvd,iron,blender,mill], axis=1, sort=False)
...
BSAhourly_1['washing'] = BSAhourly_1['washing'].astype(float)
BSAhourly_1['water'] = BSAhourly_1['water'].astype(float)
BSAhourly_1['dvd'] = BSAhourly_1['dvd'].astype(float)
BSAhourly_1['blender'] = BSAhourly_1['blender'].astype(float)
BSAhourly_1['iron'] = BSAhourly_1['iron'].astype(float)
BSAhourly_1['mill'] = BSAhourly_1['mill'].astype(float)
...
l6,=axs[0].plot(mean['local_hour'], mean['phone_charger'],linewidth=1)
l7,=axs[0].plot(mean['local_hour'], mean['washing'],linewidth=1)
#plt.fill_between(mean['local_hour'], mean['water_heater'], color='wheat',alpha=.3)

l8,=axs[0].plot(mean['local_hour'], mean['water'],linewidth=1)
#plt.fill_between(mean['local_hour'], mean['mixer'], color='rosybrown',alpha=.3)
l9,=axs[0].plot(mean['local_hour'], mean['dvd'],linewidth=1)
l10,=axs[0].plot(mean['local_hour'], mean['blender'],linewidth=1)

l11,=axs[0].plot(mean['local_hour'], mean['iron'],linewidth=1)
l12,=axs[0].plot(mean['local_hour'], mean['mill'],linewidth=1)
...
axs[1].plot(mean_1['local_hour'], mean_1['washing'],linewidth=1)
#plt.fill_between(mean_1['local_hour'], mean_1['water_heater'], color='wheat',alpha=.3)


axs[1].plot(mean_1['local_hour'], mean_1['water'],linewidth=1)
axs[1].plot(mean_1['local_hour'], mean_1['dvd'],linewidth=1)
axs[1].plot(mean_1['local_hour'], mean_1['blender'],linewidth=1)
axs[1].plot(mean_1['local_hour'], mean_1['iron'],linewidth=1)
axs[1].plot(mean_1['local_hour'], mean_1['mill'],linewidth=1)
...
axs[0].set_ylim([0, 40])
axs[1].set_ylim([0, 40])
ax.legend(handles = [l0,l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12] , labels=['Demand', 'LL', 'indoor bulb', 'outdoor bulb', 'radio', 'tv', 'phone charger','washing machine','water pump','dvd','blender','iron','mill'],loc='upper right',bbox_to_anchor=(1.55, 0.98),fancybox=False, shadow=False, ncol=1)
plt.savefig('per_app_comp_s16.png',dpi=600,bbox_inches="tight")
plt.show()
